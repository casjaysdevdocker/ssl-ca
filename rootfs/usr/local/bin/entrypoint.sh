...
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for ssl-ca
...
SERVICE_USER="${SERVICE_USER:-$ssl-ca}"  # execute command as another user
SERVICE_GROUP="${SERVICE_GROUP:-ssl-ca}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for ssl-ca"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
